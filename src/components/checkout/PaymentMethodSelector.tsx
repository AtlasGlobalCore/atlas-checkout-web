...
// Responsive payment method buttons with i18n.
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  CreditCard, QrCode, Bitcoin, Landmark, BarChart3, Smartphone, Building,
};
        ...
        {enabledMethods.map((method) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.5 sm:gap-3 rounded-xl border-2 px-3 sm:px-4 py-3 sm:py-3.5 text-left transition-all duration-200 min-h-[52px] ${
                isSelected
            ...
            >
              ...
              >
                {IconComponent ? (
                  ...
                  <div className="h-4 w-4 sm:h-5 sm:w-5 rounded-full bg-current opacity-50" />
                )}
              </div>

              <div className="flex-1 min-w-0">
                <p className={`text-xs sm:text-sm font-semibold transition-colors ${isSelected ? "text-slate-900" : "text-slate-700"}`}>
                  {method.label}
                </p>
                {method.description && (
